...
function setFlyDirection() {
    ...
    if (component) {
        ...
        const values = [0.7103407759048304, 6.124857462143081e-16, -3.250749294676129e-16, 0];
...
function updateMovement(timestamp) {
    ...
    let dx = (keyState.arrowup - keyState.arrowdown) * (movementSpeed * elapsed) / 1000;
    let dy = (keyState.arrowright - keyState.arrowleft) * (movementSpeed * elapsed) / 1000;
    moveTank(dx, dy);
...
function adjustAltitude() {
    ...
    let component = TankMovements();
    if(component) {
        component[Object.keys(component)[2]] += (keyState.j - keyState.k) * altitudeSpeed;
    }}
...
(function() {
    ...
    const pressKey = (keyCode) => {
        dispatchEvent(new KeyboardEvent("keydown", { code: keyCode }));
        dispatchEvent(new KeyboardEvent("keyup", { code: keyCode }));
    };

    let config = {
        clicker: {
            DOUBLE_ARMOR: { enabled: false, delay: 1000, code: "Digit2" },
            DOUBLE_DAMAGE: { enabled: false, delay: 1500, code: "Digit3" },
            FIRST_AID: { enabled: false, delay: 2000, code: "Digit1" },
            MINE: { enabled: true, delay: 0, multiplier: 10, code: "Digit5" },
            NITRO: { enabled: false, delay: 2500, code: "Digit4" }
        }
    };

    startAction = (action) => {
        const actionConfig = config.clicker[action];
        if (actionConfig.enabled) {
            setInterval(() => {
                pressKey(actionConfig.code);
            }, actionConfig.delay);
        }
    };
})();
